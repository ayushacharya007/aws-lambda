...
def handler(event, context):
    ...
    try:
        print(f"Lambda started. Processing {len(event['Records'])} SQS records")
        ...
        for record in event['Records']:
            print("Parsing SQS message body...")
            print(f"SQS Record: {record}")
            # Parse the SQS message body which contains the S3 notification
